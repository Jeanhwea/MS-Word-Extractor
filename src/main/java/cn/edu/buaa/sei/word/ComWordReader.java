...
public abstract class ComWordReader {
    ...
    // 构造一个JSON的内容节点
    private WordTextNode allocWordTextNode(String text) {
        return new WordTextNode(text);
    }

    // 构造一个JSON的标题节点
    private WordTitleNode allocWordTitleNode(int level, String title) {
        ...
        for (WordParagraph para : paralist) {
            ...
            if (this.style2Level(strStyle) > 0) {
                ...
                if (current_level == level + 1) {
                    item = allocWordTitleNode(current_level, text);
                ...
                if (!text.trim().isEmpty()) {
                    addContent(allocWordTextNode(text), item);
                }
            }
        }
    }
